...
fn advance_reveals_first_line() {
    ...
    state.apply(Intent::Advance).expect("advance failed");
...
fn advance_past_the_last_line_marks_dialogue_complete() {
    ...
    state.apply(Intent::Advance).unwrap(); // surface "Hi."
    assert!(!state.is_done());

    state.apply(Intent::Advance).unwrap(); // consume it; runner closes
    assert!(state.is_done());
